...
from .models import EGCN, HGDC, EMOGI, MTGCN, GCN, GAT, GraphSAGE, GIN, ChebNet, FocalLoss
...
def plot_venn_diagram():
    ...
    # Define file paths
    file_paths = {
        "EGCN": "results/EGCN_CPDB_predicted_driver_genes_epo1027_2048.csv",
...
def generate_kde_and_curves(logits, node_names, degree_counts_above, degree_counts_below, labels, train_mask, args):
    """
    Generates KDE plot comparing EGCN score ranks with KCG interaction ranks, 
    ...
    plt.yticks(fontsize=8)
    plt.xlabel('EGCN score rank', fontsize=10, labelpad=10)
...
def plot_model_performance(args):
    ...
    models = ["EGCN", "HGDC", "EMOGI", "MTGCN", "GCN", "GAT", "GraphSAGE", "GIN", "ChebNet"]
    networks = ["CPDB", "STRING", "HIPPIE"]

    # AUPRC values for ONGene and OncoKB for each model (rows: models, cols: networks)
    auroc = [
        [0.9652, 0.9578, 0.9297],  # EGCN EGCN & 0.9652 & 0.9783 & 0.9578 & 0.9738 & 0.9297 & 0.9597 \\
